...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (styleBordered) styleBordered.addEventListener('change', function () {
    ...
    if(styleBorderless) styleBorderless.addEventListener('change', function () {
    ...
    function updateTableStyle() {
        ...
        const theme = document.querySelector('input[name="tableTheme"]:checked');
        if (theme) {
        ...
        // Get size (sm, nano)
        const size = document.querySelector('input[name="tableSize"]:checked');
        if (size) {
            classes.push(size.value);
        }

        // Get accent color for the whole table
        const accent = document.getElementById('tableAccent');
        if (accent) {
            classes.push(accent.value);
        }

        // Apply header accent using classes only, not inline styles
        const headerAccentValue = headerAccent;
        ...
        if (headerAccentValue) {
            tableHeader.classList.add(headerAccentValue.value);
            // If we're using a dark background, make the text white
            if (['bg-primary', 'bg-secondary', 'bg-success', 'bg-danger', 'bg-dark'].includes(headerAccentValue.value)) {
                tableHeader.classList.add('text-white');
            }
        }

        // Get caption position (now using radio buttons)
        const captionPosition = document.querySelector('input[name="captionPosition"]:checked');
        if (captionPosition) {
            stylePreviewTable.classList.remove('caption-top', 'caption-bottom');
            stylePreviewTable.classList.add(captionPosition.value);
        } else {
            stylePreviewTable.classList.remove('caption-top', 'caption-bottom');
        }

        // Check if we should show row contextual classes
        if (showRowClasses && showRowClasses.checked) {
        ...
        if (headerAccentValue) {
            displayText += `\n\n<!-- Additional header styling -->\n<thead>\n  <tr class="${headerAccentValue.value}${['bg-primary', 'bg-secondary', 'bg-success', 'bg-danger', 'bg-dark'].includes(headerAccentValue.value) ? ' text-white' : ''}">\n    <!-- your header cells here -->\n  </tr>\n</thead>`;
        }
        ...
        if(generatedClasses) generatedClasses.textContent = displayText;
        codeClasses.textContent = displayClasses;
    }

    // Add event listeners to all form controls
    if(tableStyleForm) tableStyleForm.addEventListener('change', updateTableStyle);

    // Copy button functionality
    if(copyStyleBtn) copyStyleBtn.addEventListener('click', function () {
    ...
    if(resetStylesBtn) resetStylesBtn.addEventListener('click', function () {
        // Reset form controls
